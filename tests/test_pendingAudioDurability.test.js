  ...
  return {
    ...
    where(field) {
      return {
        equals(value) {
          return {
            async first() { return [...rows.values()].find((row) => row[field] === value); },
            async toArray() { return [...rows.values()].filter((row) => row[field] === value); },
            async count() { return [...rows.values()].filter((row) => row[field] === value).length; }
          };
        },
        anyOf(values) {
          return {
            and(predicate) { return { async toArray() { return [...rows.values()].filter((row) => values.includes(row[field]) && predicate(row)); } }; },
            async toArray() { return [...rows.values()].filter((row) => values.includes(row[field])); },
            async count() { return [...rows.values()].filter((row) => values.includes(row[field])).length; }
          };
        }
      };
    }
...
function loadManager(seed = []) {
  ...
  manager.dataStorage = { db: { pendingAudio } };
  return { manager, pendingAudio };
}
...
describe('PendingAudioManager offline durability', () => {
  test('does not prune required recordings because they are old or over maxCount', async () => {
    ...
    const { manager, pendingAudio } = loadManager(seed);
    await manager.prune({ maxCount: 30, maxAgeDays: 7 });
    expect(await pendingAudio.count()).toBe(31);
  });

  test('prunes only recordings explicitly marked disposable', async () => {
    const old = new Date('2020-01-01T00:00:00Z');
    const { manager, pendingAudio } = loadManager([
      { id: 1, timestamp: old, status: 'failed', disposable: false, transcriptPersisted: false },
      { id: 2, timestamp: old, status: 'completed', disposable: true, transcriptPersisted: true }
    ]);
    await manager.prune({ maxCount: 1, maxAgeDays: 0 });
    expect(await pendingAudio.get(1)).toBeTruthy();
    expect(await pendingAudio.get(2)).toBeUndefined();
  });

  test('deletes raw audio immediately after transcript persistence is confirmed', async () => {
    const { manager, pendingAudio } = loadManager([
      { id: 7, sourceId: 'src-7', audioBlob: new Blob(['voice']), status: 'completed', disposable: false, transcriptPersisted: false, curationId: null }
    ]);
    await manager.markTranscriptPersisted('src-7', { curationId: 'cur_1' });
    expect(await pendingAudio.get(7)).toBeUndefined();
  });

  test('associates matching draft recordings with a saved curation without deleting them', async () => {
    const { manager, pendingAudio } = loadManager([
      { id: 1, draftId: 10, curationId: null, disposable: false },
      { id: 2, draftId: 10, curationId: null, disposable: false },
      { id: 3, draftId: 11, curationId: null, disposable: false }
    ]);
    const count = await manager.associateWithCuration({ draftId: 10 }, 'cur_saved');
    expect(count).toBe(2);
    expect(await pendingAudio.get(1)).toMatchObject({ curationId: 'cur_saved' });
    expect(await pendingAudio.get(2)).toMatchObject({ curationId: 'cur_saved' });
    expect(await pendingAudio.get(3)).toMatchObject({ curationId: null });
    expect(await pendingAudio.count()).toBe(3);
  });
});
