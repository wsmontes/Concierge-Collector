...
The Concierge Collector application currently uses **`/api/restaurants/batch`** for synchronization. However, the server offers **four different sync approaches**, each with specific use cases and data formats.
...
### � Option 2: `/api/restaurants/sync` (Bulk Synchronization)

**Purpose:** Atomic create/update/delete operations in a single transaction  
**Current Usage:** ✅ Implemented but not currently used by Concierge Collector  
**Database Target:** `restaurants`, `concepts`, `restaurant_concepts` tables
...
### �🟢 Option 3: `/api/curation/json` (Recommended)
