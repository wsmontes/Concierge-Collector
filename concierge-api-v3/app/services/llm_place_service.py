...
class LLMPlaceService:
    ...
    def fetch_google_place_details(
        self, place_id: str, language: str = "pt-BR", region: str = "BR"
    ...
    def fetch_google_place_photos(
        self, place_id: str, max_photos: int = 10, language: str = "pt-BR"
    ...
    def update_entity_with_google_data(
        self, entity_id: str, google_data: Dict[str, Any], force_update: bool = False
    ) -> bool:
        """
    ...
    def get_restaurant_snapshot(
        ...
        if include_google_places and place_id:
            ...
            if not entity:
                ...
                needs_google_fetch = True
                self.logger.info("Entity missing rating, fetching from Google")
            else:
                # Use entity data
        ...
        if google_data:
            opening_hours_block, status_block = self.build_opening_hours_block(
                google_data, reference_datetime_iso, timezone
            )
    ...
    def get_restaurant_availability(
        ...
        if snapshot.opening_hours and snapshot.opening_hours.regular_hours:
            for day, periods in snapshot.opening_hours.regular_hours.items():
                ...
                availability_by_day[day] = {"is_open": is_open, "periods": periods_dict}
