...
def test_hybrid_search_special_characters():
    """Location with regex metacharacters (e.g. 'São Paulo (Centro) $$$') must not crash."""
    ...
    try:
        ...
        request = HybridSearchRequest(
            query="pizza",
            location="São Paulo (Centro) $$$",  # regex metacharacters: (), $, space
        )
        ...
        # Must complete without error and return valid results
        assert response.total_results >= 0
        assert response.query == "pizza"

        # Verify location was properly escaped in the entity query
        call_kwargs = mock_db.entities.find.call_args[0][0]
        or_clauses = call_kwargs.get("$or", [])
        assert len(or_clauses) > 0, "Expected location $or clauses to be present"

        for clause in or_clauses:
            for field_key, regex_condition in clause.items():
                escaped_location = regex_condition["$regex"]
                # Parentheses should be escaped
                assert "(" not in escaped_location or "\\(" in escaped_location, \
                    f"Unescaped parenthesis in regex: {escaped_location!r}"
                # Dollar signs should be escaped
                assert "$" not in escaped_location or "\\$" in escaped_location, \
                    f"Unescaped dollar sign in regex: {escaped_location!r}"
