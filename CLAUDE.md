...
- ⚠️ **Só o nginx pode escutar em 0.0.0.0.** Next e uvicorn precisam bindar em **127.0.0.1** (portas internas 3000 e 8000). Se algum deles escutar em `0.0.0.0`, o Render detecta aquilo como "additional ports" do serviço e passa a rotear parte do tráfego **direto para o processo, furando o nginx**: a mesma rota passa a responder 200 no uvicorn (via nginx) e 404/502 sem cabeçalhos CORS (via Next), o que o browser reporta como "bloqueado por CORS" e erro intermitente. Sintoma no log de boot: `==> Detected service running on port 10000 with additional ports HTTP:3000` — se essa linha aparecer, a invariante foi quebrada.
...
- Env vars vivem no dashboard do Render (só nomes). **Atenção ao ler via API:** `GET /services/{id}/env-vars` **pagina em 20 por padrão** — sem `?limit=100` a leitura trunca e um merge read-modify-write apagaria o resto. Escrever sempre por chave (`PUT /env-vars/{key}`), que é aditivo (verificado) — **mas o `PUT` não reinicia o serviço**: o processo continua com o valor antigo até um deploy explícito (`POST /v1/services/{id}/deploys`). Verificado em 2026-09-15, quando duas vars novas foram salvas e só passaram a valer depois do deploy disparado à mão.
  - API/captura: `MONGODB_URL`, `MONGODB_DB_NAME`, `API_SECRET_KEY`, `ADMIN_API_KEYS`, `ADMIN_EMAILS`, `JWT_SIGNING_SECRET`, `CATALOG_CURSOR_SECRET`, `DISTRIBUTION_CURSOR_SECRET`, `OPENAI_API_KEY`, `GOOGLE_PLACES_API_KEY`, `GOOGLE_OAUTH_CLIENT_ID/SECRET`, `GOOGLE_OAUTH_REDIRECT_URI`, `CORS_ORIGINS`, `TRUSTED_CALLBACK_ORIGINS`, `ENVIRONMENT`, `LOG_LEVEL`, `FRONTEND_URL(_PRODUCTION)`, `OPS_LOGIN_KEY`, `OPS_LOGIN_SUBJECT`.
    - `OPS_LOGIN_KEY` + `OPS_LOGIN_SUBJECT` habilitam `POST /api/v3/auth/ops-login`: acesso **sem Google** para qualificar as jornadas em produção, já que o Collector e o Admin só aceitam sessão nascida do OAuth. A chave viaja no **corpo** (nunca em query — o Render registra a request line) e o endpoint é fail-closed: sem as duas variáveis responde 404, chave errada recebe a MESMA resposta, e o sujeito precisa já existir em `users` com `authorized: true` e `role: admin` — o caminho substitui a prova de identidade, nunca a autorização (não cria nem promove ninguém). Com `{"redirect": true}` ele devolve 303 para `FRONTEND_URL_PRODUCTION` com a sessão no fragmento, que é como o Collector consome o retorno do Google; sem `redirect`, devolve os tokens em JSON e os mesmos cookies HttpOnly, que é o que `/auth/start` do Admin lê para abrir o handoff. Emitir sessão via esta rota é auditável em log (`[OpsLogin] Session issued for <email>`); a chave nunca aparece em log.
    - `CATALOG_CURSOR_SECRET` e `DISTRIBUTION_CURSOR_SECRET` são **obrigatórios em produção** e não têm default utilizável: o config devolve `""` e o guard só levanta no *uso* (`RuntimeError: <VAR> não configurado em produção`). Sem eles, a lista de Curations do Admin (scan assinado) e o cursor do endpoint interno do worker caem em 500→503, e a paginação de distribuição para consumidor idem. Chaves de cursor são independentes entre si e de JWT/API key — uma por finalidade.
