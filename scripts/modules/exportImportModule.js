...
class ExportImportModule {
     ...
     * Also validates if items marked as "remote" actually exist on the server
     * @returns {Promise<{localOnlyRestaurants: Array, localOnlyCurators: Array, fixedMisTaggedItems: number}>} 
     */
    async verifyLocalData() {
        try {
            console.log('Verifying local-only data and validating remote tags...');
            ...
            if (typeof syncService !== 'undefined') {
                try {
                    ...
                    // If server is available, fetch all restaurant IDs from remote
                    if (remoteServerAvailable) {
                        serverRestaurantIds = await this.fetchServerRestaurantIds();
                        console.log(`Retrieved ${serverRestaurantIds.length} restaurant IDs from server for verification`);
                    }
                } catch (error) {
            ...
            if (!remoteServerAvailable) {
                ...
                this.safeHideLoading();
                return { localOnlyRestaurants: [], localOnlyCurators: [], fixedMisTaggedItems: 0 };
            }
            ...
            // Create a set of server IDs for faster lookups
            const serverIdSet = new Set(serverRestaurantIds);
            
            // Find local-only curators - no change to this logic
            ...
            // Find local-only restaurants - using the traditional check
            ...
            // Find incorrectly tagged restaurants (marked as remote but not on server)
            const misTaggedRestaurants = [];
            for (const restaurant of restaurantsById.values()) {
                // Check any restaurant tagged as remote/server but missing from server
                if (restaurant.origin === 'remote' || restaurant.serverId) {
                    // If it has a serverId, check if that ID exists on the server
                    if (restaurant.serverId && !serverIdSet.has(restaurant.serverId)) {
                        misTaggedRestaurants.push(restaurant);
                        console.log(`Restaurant ${restaurant.name} (ID: ${restaurant.id}) incorrectly tagged as remote - not found on server`);
                    }
                }
            }
            console.log(`Found ${misTaggedRestaurants.length} incorrectly tagged restaurants`);
            ...
            this.safeHideLoading();
            return { 
                localOnlyRestaurants, 
                localOnlyCurators,
                fixedMisTaggedItems: fixedCount
            };
        } catch (error) {
            this.safeHideLoading();
    ...
    /**
     * Fetches all restaurant IDs from the server for verification
     * @returns {Promise<Array>} Array of server restaurant IDs
     */
    async fetchServerRestaurantIds() {
        try {
            console.log('Fetching restaurant IDs from server for verification...');
            
            // Use a special endpoint that only returns IDs for efficiency
            // If this endpoint doesn't exist, modify to extract IDs from full restaurant data
            const response = await fetch('https://wsmontes.pythonanywhere.com/api/restaurants?fields=id');
            
            if (!response.ok) {
                // If the specialized endpoint is not available, try the regular endpoint
                console.log('ID-only endpoint not available, using full restaurant data endpoint');
                const fullResponse = await fetch('https://wsmontes.pythonanywhere.com/api/restaurants');
                
                if (!fullResponse.ok) {
                    throw new Error(`Server error: ${fullResponse.status} ${fullResponse.statusText}`);
                }
                
                const restaurants = await fullResponse.json();
                // Extract just the IDs from the restaurant objects
                return restaurants.map(restaurant => restaurant.id || restaurant.serverId).filter(id => id);
            }
            
            const data = await response.json();
            
            // Depending on the API response format, extract the IDs
            if (Array.isArray(data)) {
                // If response is an array of objects with id property
                return data.map(item => item.id || item.serverId).filter(id => id);
            } else if (data && Array.isArray(data.ids)) {
                // If response has an ids array property
                return data.ids;
            }
            
            console.warn('Unexpected server response format for restaurant IDs');
            return [];
            
        } catch (error) {
            console.error('Error fetching server restaurant IDs:', error);
            return [];
        }
    }
    
    /**
     * Fixes restaurants that are incorrectly tagged as remote
     * @param {Array} misTaggedRestaurants - Array of incorrectly tagged restaurants
     * @returns {Promise<number>} - Number of fixed items
     */
    async fixMisTaggedRestaurants(misTaggedRestaurants) {
        try {
            console.log(`Fixing ${misTaggedRestaurants.length} incorrectly tagged restaurants...`);
            ...
            console.error('Error fixing mistagged restaurants:', error);
            return 0;
        }
    }
