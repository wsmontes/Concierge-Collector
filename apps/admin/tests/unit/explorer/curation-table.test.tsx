...
import { DEFAULT_CURATION_COLUMNS } from '../../../src/content/curation-columns'
...
describe('CurationTable', () => {
  ...
  test('header checkbox is checked when every loaded row is selected and forwards the toggle', () => {
    ...
    render(<CurationTable
      columns={DEFAULT_CURATION_COLUMNS}
      height={600}
      isSelected={() => true}
  ...
  test('Enter opens the active row and a checkbox click never does', () => {
    ...
    expect(onOpenRow).toHaveBeenCalledWith(expect.objectContaining({ curation_id: 'curation-2' }))
  })
})
