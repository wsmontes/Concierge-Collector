...
describe('consumer credential endpoints', () => {
  ...
  test('returns a secret once with no-store and forwards only valid input', async () => {
    ...
    const endpoint = credentialEndpoints().find(({ method, path }) => method === 'post' && path === '/admin/v1/applications/:id/credentials')
  ...
  test('rotates with an overlap window and returns the new secret once', async () => {
    rotateCredential.mockResolvedValueOnce({ credential: { id: 'cred-2', prefix: 'b'.repeat(12) }, secretOnce: 'cck_rotated' })
    const { credentialEndpoints } = await import('../../../src/payload/endpoints/credentials')
    ...
    expect(response.status).toBe(201)
    expect(response.headers.get('cache-control')).toBe('private, no-store')
    expect(await response.json()).toEqual({ credential: { id: 'cred-2', prefix: 'b'.repeat(12) }, secret_once: 'cck_rotated' })
    expect(rotateCredential).toHaveBeenCalledWith(
      '0123456789abcdef01234567',
      expect.objectContaining({ credentialId: '0123456789abcdef01234567', actorId: 'admin-1', idempotencyKey: 'rotate-1', overlapUntil: new Date('2026-08-21T00:00:00.000Z') }),
      expect.anything(),
    )
  })

  test('rejects a rotate payload without an overlap window', async () => {
    const { credentialEndpoints } = await import('../../../src/payload/endpoints/credentials')
    const endpoint = credentialEndpoints().find(({ method, path }) => method === 'post' && path === '/admin/v1/credentials/:id/rotate')
    const response = await endpoint!.handler(requestFor('https://admin.example.test/api/admin/v1/credentials/0123456789abcdef01234567/rotate', {
      method: 'POST', routeParams: { id: '0123456789abcdef01234567' },
