...
function queryValue(request: PayloadRequest, key: string): string | null {
  ...
  try {
    return new URL((request as unknown as Request).url).searchParams.get(key)
  } catch {
    return null
  }
}
...
function idOf(value: RecordValue): string {
...
function publishModel(request: PayloadRequest) {
  ...
  return model as unknown as {
    find(query: RecordValue): {
      sort(sort: RecordValue): {
        limit(limit: number): { lean(): Promise<RecordValue[]> }
      }
    }
  }
}
...
async function recentBulkOperations(request: PayloadRequest, actorId: string) {
  ...
  const items = page.map((parent) => {
    ...
    return {
      ...
      cancellable: children.some((child) => CANCELLABLE.has(String(child.status))),
      collections: ids.map((collectionId) => directory.get(collectionId) ?? { id: collectionId, title: 'Collection', slug: '' }),
      createdAt: parent.createdAt,
      updatedAt: aggregated?.latestUpdatedAt ?? parent.updatedAt,
    }
  })

  return {
    items,
    nextCursor: parents.length > PAGE_LIMIT ? encodeCursor(idOf(page[page.length - 1])) : null,
  }
}

async function recentPublishJobs(request: PayloadRequest, actorId: string) {
  const after = decodeCursor(request)
  const rows = await publishModel(request).find({
    actorId,
    ...(after ? { _id: { $lt: after } } : {}),
  ...
  return {
    items: page.map((row) => {
      ...
      return {
        id: idOf(row),
        collection: directory.get(collectionId) ?? { id: collectionId, title: 'Collection', slug: '' },
        targetVersion: Number(row.targetVersion),
        status: String(row.status),
        checkpoint: row.checkpoint ?? null,
        selectedCount: row.selectedCount ?? null,
        confirmedUnavailableCount: Number(row.confirmedUnavailableCount ?? 0),
...
export function operationsAdminEndpoints(): Endpoint[] {
  return [
    {
      method: 'get',
      path: '/admin/v1/operation-history',
